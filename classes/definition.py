...
class GrabDefinition:
    ...
    def grab_word_definition(word):
        ...
        # URL in which would be used to gather the definition
        # data for the self.hangman_word if the user decides
        # to use the hint token
        url = f"https://dictionary-data-api.\
p.rapidapi.com/definition/{word}"
        # Header that would be used alongside with the url
        # to gather definition data of
        # the self.hangman_word, and holds the secret api
        # key which is being held
        # in the env.py file
        ...
        try:
            if response.status_code == 200:
                ...
                if meanings:
                    ...
                    # Display definition to player and hide
                    # the word by replacing
...
{definition.replace(word, '(hidden correct word)')}" + Fore.RESET)
    ...
    def grab_country_data(country):
        ...
        # URL in which would be used to gather the definition
        # data for the self.hangman_word
...
/geography/v1/country/name/{country}"
        ...
        # to gather definition data of
        # the self.hangman_word, and holds the secret api
        # key which is being held in the env.py file
        headers = {'X-RapidAPI-Key': api_key}
        response = requests.get(url, headers=headers)
        # If the data is grabbed successfully, do the following
        try:
