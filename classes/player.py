...
class Player(AsciiArtMixin, ClearTerminalMixin):
    ...
    def collect_info(self):
        ...
        print(Fore.LIGHTYELLOW_EX + "Welcome! could you be the one to save this poor guy from a \ngruesome death? i hope so! fill in your name and location to\nsee the how to play guide.\n")
        while True:
            ...
            try: 
                ...
                if len(self.name_of_player) == 0:
                    ...
                    print(Fore.LIGHTYELLOW_EX + "Your name does not have numbers in it... right??")
                else:
                    break
            except KeyboardInterrupt:
                print(Fore.LIGHTYELLOW_EX + "KeyboardInterrupt (ctrl + c) is not allowed during input. Please try again.")

        while True:
            # Try statement to avoid quitting game when player attempts keyboard interruption (ctrl + c) 
            try:
                self.location_of_player = input(
                    Fore.LIGHTCYAN_EX + "Which country/city are your from?\n>>> ")
                if len(self.location_of_player) == 0:
                    print(Fore.LIGHTYELLOW_EXW + "Please enter your location..")
                elif any(char.isdigit() for char in self.location_of_player):
                    print(Fore.LIGHTYELLOW_EX + "Please do not use numbers..")
                else:
                    self.how_to_play()
                    break
            except KeyboardInterrupt:
